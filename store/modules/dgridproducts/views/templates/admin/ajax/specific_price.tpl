...
<div class="content_form p_{$ps_version|replace:'.':''|escape:'quotes':'UTF-8'}">
	<h3>{l s='Specific prices' mod='dgridproducts'}</h3>
	<table id="specific_prices_list" class="table table-bordered">
		<thead>
		<tr>
			<th>{l s='Rule' mod='dgridproducts'}</th>
			<th>{l s='Combination' mod='dgridproducts'}</th>
			{if $multi_shop}<th>{l s='Shop' mod='dgridproducts'}</th>{/if}
			<th>{l s='Currency' mod='dgridproducts'}</th>
			<th>{l s='Country' mod='dgridproducts'}</th>
			<th>{l s='Group' mod='dgridproducts'}</th>
			<th>{l s='Customer' mod='dgridproducts'}</th>
			<th>{l s='Fixed price' mod='dgridproducts'}</th>
			<th>{l s='Impact' mod='dgridproducts'}</th>
			<th>{l s='Period' mod='dgridproducts'}</th>
			<th>{l s='From (quantity)' mod='dgridproducts'}</th>
			<th>{l s='Action' mod='dgridproducts'}</th>
		</tr>
		</thead>
		<tbody>
			{$specific_prices_html|escape:'quotes':'UTF-8'}
		</tbody>
	</table>
	<div>
