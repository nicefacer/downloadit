...
{block name="td_content"}
    ...
    {else}
        ...
        <div
            ...
            {if !in_array($params.type, ['combinations', 'features', 'meta_tags', 'additional_setting_product', 'specific_price'])}
        ...
        {else}
            ...
            {if $params.type == 'specific_price'}
                <a data-id="{$tr.id|escape:'quotes':'UTF-8'}" class="button btn btn-default viewSpecificPrices {if $tr.has_specific_price}has_specific_price{/if} {if $tr.has_group_specific_price}has_group_specific_price{/if}" href="#" title="{l s='Specific prices' mod='dgridproducts'}">%</a>
            {/if}
        {/if}
    {/if}
    {if $ps_v < 1.6}</td>{/if}
