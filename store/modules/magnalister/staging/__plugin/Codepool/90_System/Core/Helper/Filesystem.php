...
class ML_Core_Helper_Filesystem {
    ...
    /**
     * alias for MLFilesystem::glob
     * @param string $sPattern
     * @param int $iFlags
     * @return array
     */
    public function glob($sPattern, $iFlags = 0){
        return MLFilesystem::glob($sPattern, $iFlags);
    }
    ...
    public function mv($sSrcPath, $sDstPath) {
        ...
        if (
            ...
            && count($this->glob(is_dir($sSrcPath) ? $sDstPath : dirname($sDstPath).'/*.*')) == 0
        ) {
            $this->rm($sDstPath);
        }
...
//         $this->write(is_dir($sSrcPath) ? $sDstPath : dirname($sDstPath));
    ...
    public function cp($sSrcPath, $sDstPath, $blCleanDst = true) {
        ...
        if (is_dir($sSrcPath)) {
            ...
            foreach($this->glob($sSrcPath.'/*') as $sPath) {
    ...
    public function rm($sPath) {
        ...
        if ($this->isWritable($sPath)) {
            if (file_exists($sPath)) {
                if (is_dir($sPath)) {
                    foreach ($this->glob($sPath.'/*') as $sSubPath) {
                        $this->rm($sSubPath);
                    }
                    @rmdir($sPath);
                } else {
