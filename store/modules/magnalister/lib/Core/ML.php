...
final class ML {
    ...
    public static function isUpdate() {
        if (self::$blUpdate === null) {
            self::$blUpdate = self::isInstalled() || file_exists(MLFilesystem::getLibPath('Update'));
            if (self::$blUpdate && !file_exists(MLFilesystem::getLibPath('Update'))) {
                MLHelper::getFilesystemInstance()->write(MLFilesystem::getLibPath('Update'), date('Y-m-d H:i:s'). "\n");
            }
        }
    ...
    public static function gi() {
        if (self::$oInstance === null) {
            try {
                ...
                if (!file_exists(MLFilesystem::gi()->getLibPath('ForceCleanCache'))) {
                    // try to clean cache and rebuild application again
                    @file_put_contents(MLFilesystem::gi()->getLibPath('ForceCleanCache'), '');
                    try {
                        MLCache::gi()->flush();
            ...
            MLHelper::getFilesystemInstance()->rm(MLFilesystem::gi()->getLibPath('ForceCleanCache'));
        }
