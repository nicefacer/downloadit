...
class ML_Hitmeister_Controller_Hitmeister_Prepare_Apply_Form extends ML_Form_Controller_Widget_Form_PrepareAbstract {
	...
	protected function triggerBeforeFinalizePrepareAction() {
        ...
        if (isset($aMatching['variationgroups'])) {
            ...
            foreach ($aMatching as $key => &$value) {
                ...
                if ($value['Required'] && $value['Code'] !== 'category' && $value['Code'] !== 'freetext') {
                    $sAttributeValue = $this->oProduct->getAttributeValue($value['Code']);
                    if (!isset($sAttributeValue)) {
                        $aErrors[] = $key . MLI18n::gi()->get('hitmeister_prepare_variations_error_missing_value');
                    }
                }
    ...
    public function triggerAfterField(&$aField) {
        ...
        if (!isset($aField['value'])) {
            ...
            $aPrimaryCategories = $this->oPrepareList->get('primarycategory');
            if ($sName === 'variationgroups.value') {
                $aField['value'] = isset($aPrimaryCategories['[' . $sProductId . ']']) ? $aPrimaryCategories['[' . $sProductId . ']'] : '';
            } else {
                ...
                if (count($aNames) == 4 && strtolower($aNames[3]) === 'code') {
                    ...
                    if (!isset($aValue['[' . $sProductId . ']']) || $aPrimaryCategories['[' . $sProductId . ']'] !== $aNames[1]) {
    ...
    protected function getAttributeValues($sIdentifier, $sCustomIdentifier, $sAttributeCode, $bFreeText = false) {
        $sProductId = $this->getProductId();
        $sPrimaryCategory = $this->oPrepareList->get('primarycategory');
        if ($sPrimaryCategory['[' . $sProductId . ']'] === $sIdentifier) {
            $aShopVariation = $this->oPrepareList->get('shopvariation');
