...
class ML_MeinPaket_Controller_MeinPaket_Prepare_Variations extends ML_Form_Controller_Widget_Form_ConfigAbstract {

    protected $aParameters = array('controller');
    private $shopAttributes;

    public static function getTabTitle() {
        return MLI18n::gi()->get('meinpaket_prepare_variations_title');
    }
...
//            unset($aMatching['variationgroups']);
    ...
    public function saveAction($blExecute = true) {
        if ($blExecute) {
            ...
            if (isset($aMatching['attributename'])) {
                ...
                if ($sIdentifier === 'none') {
                    MLMessage::gi()->addError(MLI18n::gi()->get('meinpaket_prepare_match_variations_attribute_missing'));
                    return;
                }

                if ($sCustomIdentifier == '') {
                    MLMessage::gi()->addError(MLI18n::gi()->get('meinpaket_prepare_match_variations_custom_ident_missing'));
                    return;
                }
            }
            ...
            if (isset($aMatching['variationgroups'])) {
                ...
                foreach ($aMatching as $key => $value) {
                    if ($value['Code'] == '') {
                        MLMessage::gi()->addError(MLI18n::gi()->get('meinpaket_prepare_match_variations_not_all_matched'));
                        return;
                    }
                ...
                $oVariantMatching->set('Identifier', $this->encodeText($sIdentifier, false))
                    ...
                    ->save();
                MLMessage::gi()->addSuccess(MLI18n::gi()->get('meinpaket_prepare_match_variations_saved'));
            } else {
                MLMessage::gi()->addError(MLI18n::gi()->get('meinpaket_prepare_match_variations_no_selection'));
            }
        }
    }
    ...
    protected function variationGroups_ValueField(&$aField) {
        ...
        $aField['values'][MLI18n::gi()->get('meinpaket_prepare_variations_groups')] = $aValues;
        $customVars = $this->getCustomVariations();
        if (count($customVars) > 0) {
            $aField['values'][MLI18n::gi()->get('meinpaket_prepare_variations_groups_custom')] = $customVars;
        }

        $aField['values']['new'] = MLI18n::gi()->get('meinpaket_prepare_variations_groups_new');
    }
    ...
    /**
     * @return ML_MeinPaket_Model_Table_MeinPaket_VariantMatching
     */
    private function getVariationDb() {
        return MLDatabase::factory('meinpaket_variantmatching');
    }
