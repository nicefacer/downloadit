...
class ML_MeinPaket_Controller_MeinPaket_Checkin_Summary extends ML_Productlist_Controller_Widget_ProductList_Selection {
    ...
    public function getPrepareData(ML_Shop_Model_Product_Abstract $oProduct) {
        if (!isset($this->aPrepare[$oProduct->get('id')])) {
            $this->aPrepare[$oProduct->get('id')] = MLDatabase::factory('meinpaket_prepare')->set('products_id', $oProduct->get('id'));
        }
