...
class ML_MeinPaket_Model_Service_AddItems extends ML_Modul_Model_Service_AddItems_Abstract {
    ...
    protected function processVariations(&$aProduct) {
        foreach ($aProduct['Variations'] as &$aVariant) {
            ...
            if (empty($varConfig)) {
                MLErrorLog::gi()->addError(null, null, MLI18n::gi()->get('meinpaket_error_checkin_variation_config_empty'), $aNewVarConfig);
                continue;
            }
            ...
            foreach ($aVariant['Variation'] as &$vSet) {
                if (!isset($varConfig[$vSet['code']])) {
                    $sMessage = MLI18n::gi()->get(
                        'meinpaket_error_checkin_variation_config_missing_nameid', 
                        array (
                            'Attribute' => $vSet['name'],
                            'SKU' =>  $aVariant['SKU'],
    ...
    protected function loadVariationMatching($variationConfiguration) {
        $varConfig = MLDatabase::factory('meinpaket_variantmatching')
        ...
        $apiVarConfig = ML::gi()->instance('controller_meinpaket_prepare_variations')->getMPVariationAttributes($identifier);
