 ...
 * -----------------------------------------------------------------------------
 */
class ML_Fyndiq_Model_Service_AddItems extends ML_Modul_Model_Service_AddItems_Abstract
{
    ...
    {
        /* @var $oHelper ML_Fyndiq_Helper_Model_Service_Product */
            ...
            if (count($aVariants) === 1) {
                ...
                foreach ($aVariants as $oVariant) {
                    if ($this->oList->isSelected($oVariant)) {
                        ...
                        $oVariantProduct['ArticleName'] = substr($oVariantProduct['ArticleName'], strpos($oVariantProduct['ArticleName'], ':') + 2);
                ...
                if (
                    (!isset($oMasterProduct['Images']) || empty($oMasterProduct['Images']))
                    && isset($oMasterProduct['Variations']) && is_array($oMasterProduct['Variations'])
                ) {
                    foreach ($oMasterProduct['Variations'] as $aVariant) {
                        if (isset($aVariant['Images']) && !empty($aVariant['Images'])) {
                            $oMasterProduct['Images'] = $aVariant['Images'];
                            break;
                        }
                    }
                }
    ...
    {
        $oPreparedItem = MLDatabase::factory('fyndiq_prepare')
    ...
    /**
     * edit master or variants that their quantity <= 0
     * @return boolean
     */
