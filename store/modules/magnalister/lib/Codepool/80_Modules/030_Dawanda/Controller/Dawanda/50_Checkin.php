...
class ML_Dawanda_Controller_Dawanda_Checkin extends ML_Productlist_Controller_Widget_ProductList_Selection{
    ...
    public function getProductListWidget() {
        if ($this->isCurrentController()) {
            if (count($this->getProductList()->getMasterIds(true))==0) {//only check current page
                MLMessage::gi()->addInfo($this->__('ML_DAWANDA_TEXT_NO_MATCHED_PRODUCTS'));
            }
