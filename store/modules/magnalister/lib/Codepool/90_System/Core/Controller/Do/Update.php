                ...
                try {
                    if (count(
                        ...
                        foreach (array(
                            ...
                            $oZip->close();
                            if (count(MLHelper::getFilesystemInstance()->glob($this->getPath('zip').$sType.'/*')) < 1) {
                                throw new Exception('Problems with archiv files.', 1462872613);
                            }
                        }
                        $this->blUseZip = true;
                        MLController::gi('widget_progressbar')->addLog('# Remote-Path: '.$this->getPath('server'));
                    }
                } catch (Exception $oEx) {
//                    echo $oEx->getMessage();
                }
...
//            echo json_encode($this->aSequences);die;
    ...
    public function callAjaxUpdate () {
        ...
        try {
            ...
            foreach ($this->prepareSequenzes(false)->aSequences['plugin'] as $sCheckSequenceType => $aCheckSequenceType) {
