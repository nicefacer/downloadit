...
class ML_Core_Helper_Remote {
    ...
    public function getFileList($sRemote) {
        if (strpos($sRemote, 'http') === 0) {
            ...
            foreach (array(
                'files.list?format=json' => '__plugin', 
                'external.list?format=json&shopsystem='.MLShop::gi()->getShopSystemName() => '__external'
            ) as $sRequest => $sType) {
                $warnings = null;
            ...
            $aLocalFiles = $this->readDir($sRemote);
            for ($i = 0; $i < count($aLocalFiles); $i++) {
                if (is_dir($aLocalFiles[$i])) {
                    $add = $this->readDir($aLocalFiles[$i].'/');
    ...
    protected function readDir ($sFolder) {
        if (defined('GLOB_BRACE')) {
            /**
             * @deprecated GLOB_BRACE is only supported for gnu os (eg. not solaris) - better use readdir for all
             * actually we use glob, for not breaking updater
             */
            $aDir = MLFilesystem::gi()->glob($sFolder.'{,.}*', GLOB_BRACE);
        } else {
            $aDir = array();
            $rDir = opendir($sFolder);
            while ($sPath = readdir($rDir)) {
                if (!in_array($sPath, array('.', '..'))) {
                    $aDir[] = $sFolder.$sPath;
                }
            }
            closedir($rDir);
