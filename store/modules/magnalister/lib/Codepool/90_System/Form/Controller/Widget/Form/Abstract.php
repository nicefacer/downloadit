...
abstract class ML_Form_Controller_Widget_Form_Abstract extends ML_Core_Controller_Abstract{
        ...
        try{
            ...
            if(isset($aAjaxData['method'])){
                ...
                if (isset($aField['type'])) {
                    MLSetting::gi()->add('aAjaxPlugin', array('dom' => array( '#'.$aField['id'].'_ajax'=> $this->includeTypeBuffered($aField))));
     ...
     * @param array $aField min: array('name'=>'fieldName') or array('realname'=>'fieldName')
     * @param string $sVector which part of field
     * @param null $sVector complete field
     * @return array normalized $aField - with i18n, template data...
     *  array(
    ...
    protected function getField($aField, $sVector = null) {
        ...
        if (!isset($this->aFields[$sName])) {
            ...
            $aField['hint']['template'] = isset($aField['hint']['template']) ? $aField['hint']['template'] : 'text';
            $aI18n = $this->getFormArray('aI18n');
            $aI18n = isset($aI18n['field'][$sName]) ? $aI18n['field'][$sName] : array();
