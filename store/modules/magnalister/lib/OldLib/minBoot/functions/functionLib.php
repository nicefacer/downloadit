 ...
 *
 * -----------------------------------------------------------------------------
 * $Id: functionLib.php 6524 2016-03-01 11:01:04Z markus.bauer $
 *
...
function toURL($a = array(), $b = array(), $forAjax = false) {
	...
	if (!empty($_GET)) {
		foreach ($_GET as $get_key => $get_value) {
			if ($get_value == MLShop::gi()->getSessionId()) {
				$a[$get_key] = $get_value;
			}
		}
	}
...
//		$u .= $k.'='.$v.'&'.($forAjax ? '' : 'amp;');
//	}
//	return MLShop::gi()->MLHttp().'?'.substr($u, 0, ($forAjax ? -1 : -5));
        $sUrl= MLHttp::gi()->getUrl($a);
        if(isset($a['ajax'])&&$a['ajax']=='true'){
