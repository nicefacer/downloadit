 ...
 *
 * -----------------------------------------------------------------------------
 * $Id: InventoryView.php 5847 2015-07-22 10:10:06Z masoud.khodaparast $
 *
...
$(document).ready(function() {
	$('#listingDelete').click(function() {
		if (($('#inventory input[type="checkbox"]:checked').length > 0) &&
			confirm(unescape(<?php echo "'".html_entity_decode(sprintf($this->__('ML_GENERIC_DELETE_LISTINGS'), $_modules[$this->magnaSession['currentPlatform']]['title']))."'"; ?>))
		) {
			$('#action').val('delete');
