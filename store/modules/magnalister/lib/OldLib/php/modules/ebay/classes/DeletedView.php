			...
			switch ($aPost['action']) {
				case 'delete': {
					...
					foreach ($itemIDs as $itemID) {
						...
						/*$pDetails = unserialize(str_replace('\\"', '"', $aPost['details'][$itemID]));
						$pID = magnaSKU2pID($sku);
						$model = '';
						if ($pID > 0) {
							$model = (string)MLDatabase::getDbInstance()->fetchOne('SELECT products_model FROM '.TABLE_PRODUCTS.' WHERE products_id=\''.$pID.'\'');
						}
						if (empty($model)) {
							$model = $sku;
						}
...
jqml(document).ready(function() {
	jqml('#listingDelete').click(function() {
		if ((jqml('#ebayinventory input[type="checkbox"]:checked').length > 0) &&
			confirm(unescape(<?php echo "'".  html_entity_decode(sprintf($this->__s('ML_GENERIC_DELETE_LISTINGS',array("'")), $_modules[$this->magnasession['currentPlatform']]['title']))."'"; ?>))
		) {
