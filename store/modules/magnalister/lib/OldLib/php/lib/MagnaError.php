 ...
 *
 * -----------------------------------------------------------------------------
 * $Id: MagnaError.php 4657 2014-09-29 14:55:21Z derpapst $
 *
...
class MagnaError {
	...
	public static function gi() {
		if (self::$instance === null) {
            ...
            try{
                foreach(MLCache::gi()->get(strtoupper(__CLASS__).'__exeptions.json') as $sSerializedException){
                    $e=new MagnaException('', 0, array(), array(), 0);
                    $e->unserialize($sSerializedException);
                    self::$instance->addMagnaException($e);
                }
                MLCache::gi()->delete(strtoupper(__CLASS__).'__exeptions.json');
            }catch(Exception $oEx){
//                echo $oEx->getMessage();
            }
		}
	...
	public function __destruct() {
        $aCache=array();
        foreach($this->exceptionCollection as $oEx){
            $aCache[]=$oEx->serialize();
        }
        MLCache::gi()->set(strtoupper(__CLASS__).'__exeptions.json',$aCache);
    ...
    public function init(){
        $this->exceptionCollection=array();
    }
		...
		if (!empty($this->exceptionCollection)) {
			...
			foreach ($this->exceptionCollection as $exc) {
				...
				foreach ($response['ERRORS'] as $error) {
					...
					if (isset($error['REPLY'])&&is_array($error['REPLY'])) {
						$error['REPLY'] = print_r($error['REPLY'], true);
					}
					if (isset($error['SUBSYSTEM']) && $error['SUBSYSTEM'] == 'PHP') {
					...
					$html .= '
							<td class="message">
								'.html_entity_decode($errorMessage).(!empty($addDebug) ? '<pre>'.$addDebug.'</pre>' : '').'
							</td>
						</tr>
						<tr>';
				}
