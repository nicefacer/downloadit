 ...
 *
 * -----------------------------------------------------------------------------
 * $Id: SimplePrice.php 4654 2014-09-29 11:12:15Z markus.bauer $
 *
...
class SimplePrice {
	...
	public function __construct($price = null, $actualCurr = null) {
                $this->currencies=  MLCurrency::gi()->getList();
		if ($actualCurr != null) {
	...
	public function getGroupPrice($groupID, $productID) {
		if (!MLDatabase::getDbInstance()->tableExists(TABLE_PERSONAL_OFFERS_BY.$groupID)) {
			return 0.0;
		}
		return (float)MLDatabase::getDbInstance()->fetchOne('
	...
	public function setPriceFromDB($pID, $mpID, $extra = '') {
		...
		$this->price = (float)MLDatabase::getDbInstance()->fetchOne('
	...
	public static function getTaxByClassID($taxClassID, $countryID = -1) {
		if ($countryID == -1) {
			...
			$countryID = (int)MLDatabase::getDbInstance()->fetchOne('
				SELECT configuration_value FROM '.TABLE_CONFIGURATION.'
				 WHERE configuration_key=\'STORE_COUNTRY\'
			');
		}
		$taxRate = MLDatabase::getDbInstance()->fetchOne(eecho('
			SELECT MAX(tax_rate)
	...
	public static function getTaxByPID($pID) {
		$taxClassID = MLDatabase::getDbInstance()->fetchOne('
	...
	public function updateCurrency($val) {
		...
		MLDatabase::getDbInstance()->update(TABLE_CURRENCIES, array(
			'value' => $val,
		), array (
