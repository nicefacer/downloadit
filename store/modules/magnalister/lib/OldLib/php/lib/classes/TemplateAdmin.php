 ...
 *
 * -----------------------------------------------------------------------------
 * $Id: TemplateAdmin.php 4304 2014-07-30 13:09:16Z derpapst $
 *
...
class TemplateAdmin {
	...
	public function __construct() {
		...
		if (array_key_exists('removeItems', $_POST) && is_array($_POST['removeItems'])) {
			...
			foreach ($tplIDs as $tID) {
				MLDatabase::getDbInstance()->delete(TABLE_MAGNA_SELECTION_TEMPLATE_ENTRIES, array('tID' => $tID));
				MLDatabase::getDbInstance()->delete(TABLE_MAGNA_SELECTION_TEMPLATES, array('tID' => $tID));
			}
		}
	}
	...
	public function renderTemplateList() {
		...
		$templates = MLDatabase::getDbInstance()->fetchArray('
		...
		foreach ($templates as $template) {
			$template['items'] = MLDatabase::getDbInstance()->fetchOne('
			    SELECT COUNT(pID)
