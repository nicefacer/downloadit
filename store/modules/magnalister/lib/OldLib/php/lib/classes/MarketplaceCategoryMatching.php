...
abstract class MarketplaceCategoryMatching {
	...
	public function __construct() {
		...
		$this->hasPlatformCol = MLDatabase::getDbInstance()->columnExistsInTable('platform', $this->getTableName());
	}
	...
	private function importMPCategories($parentID = 0) {
		...
		if ($parentID.'' == (0).'') {
			...
			MLDatabase::getDbInstance()->delete($this->getTableName(), $w);
		}
		...
		MLDatabase::getDbInstance()->batchinsert($this->getTableName(), $categories['DATA'], true);
		return true;
	}
	...
	private function importMPStoreCategories() {
		...
		MLDatabase::getDbInstance()->batchinsert($this->getTableName(), $categories, true);
		return true;
	}
	...
	private function getMPCategories($parentID = 0, $purge = false) {
		if ($purge) {
			...
			MLDatabase::getDbInstance()->delete($this->getTableName(), $where);
		}
		$validTo = gmdate('Y-m-d H:i:s', time() - $this->getCategoryValidityPeriod());

		$mpCategories = MLDatabase::getDbInstance()->fetchArray('
	...
	private function getMPCategoryPath($categoryID) {
		...
		do {
			...
			$yCP = MLDatabase::getDbInstance()->fetchRow('
		...
		$('#mpCategorySelector').jDialog({
			...
			open: function(event, ui) {
				...
				if (tbar.find('.ui-icon-arrowrefresh-1-n').length == 0) {
					var rlBtn = $('<a class="ui-dialog-titlebar-close ui-corner-all ui-state-focus ml-js-noBlockUi" '+
