...
class GenerateProductsDetailInput {
	...
	public function __construct($structure, $data = array(), $cId, $sName,$oProduct = null) {
                $this->sPrefix=MLHttp::gi()->parseFormFieldName('field['.$sName.']['.$cId.']');
        ...
        private function parseFieldName($sName){
            if(strpos($sName, '[')){
                $sAdd=substr($sName,strpos($sName, '['));
                $sName=substr($sName,0,strpos($sName, '['));
            }else{
                $sAdd='';
            }
            return $this->sPrefix==''?$sName:$this->sPrefix.'['.$sName.']'.$sAdd;
        }
...
//                new dBug(array($keys,$val,  MLRequest::gi()->data()),'',true);
		foreach ($keys as $key) {
	...
	private function getInputValue($curRowC, $isMulticolumn, $subKey = '') {
		...
		$tmppost = $this->getInputValueHelper('post', $keys);
                 ...
                 * code to fill MPN , EAN and Brand in ebay product attributes
                  */
                if(in_array($this->curInput['type'], array('text','select&text')) && $this->getProduct() !== null) {                    
                    try{
                        $sValue = MLHelper::gi('Model_Form_Type_Attribute')->fillValue($this->getProduct(),$this->curField['label']);
...
//                        MLMessage::gi()->addDebug($oExc);
	...
	private function renderInput($curRowC, $isMulticolumn) {
		...
		$inputID   = rtrim(str_replace(array('][', '[', ']') , '_', $this->parseFieldName($inputName)), '_');//use parseFieldName return value to create uniqu id
                $className   = rtrim(str_replace(array('][', '[', ']') , '_', $inputName), '_');//keep old behavior for html class name in textarea
			...
			$this->out .= '
				<script type="text/javascript">/*<![CDATA[*/
					function handleBlock_'.$dependantInputID.' () {
						var '.$dependantInputID.'_Value = jqml.trim($(\'#'.$dependantInputID.'\').val());
					...
					$(document).ready(function() {
						...
						$(\'#'.$dependantInputID.'\').blur(function() {
							...
							handleBlock_'.$dependantInputID.'();
						}).keyup(jqml.debounce(150, true, function() {
		...
		switch ($this->curInput['type']) {
			case 'text': {
				...
				$this->out .= '<input type="text" id="'.$inputID.'" name="'.$this->parseFieldName($inputName).'" value="'.$this->getInputValue($curRowC, $isMulticolumn).'" '.($this->curInputReadonly?'readonly="readonly" style="color:#555555;background-color:#DCDAD5" ':'').$class.'/>'."\n";
				break;
			}
			case 'select': {
				if (empty($class)) {
				...
				$ret = '<select id="'.$inputID.'" name="'.$this->parseFieldName($inputName).'" '.$class.'>'."\n";
			...
			case 'multipleselect': {
                ...
                $ret = '<select id="'.$inputID.'" name="'.$this->parseFieldName($inputName).'[]" '.$class.' size="5" '.
				...
				if (empty($class)) {
					...
					$class = 'class="'.implode(' ', $class).'"';
                                ...
                                if($this->curInputReadonly){
                                    $ret = '<input  type="hidden" name="'.$this->parseFieldName($inputName).'[select]" value="-6" '."/>\n";
                                }else{
                                    $ret = '<select id="'.$inputID.'_select" name="'.$this->parseFieldName($inputName).'[select]" '.$class.'>'."\n";				
                                    foreach ($this->curInput['values'] as $val => $text) {
                                            if ($default == $val) {
                                                    $sel = ' selected="selected"';
                                            } else {
                                ...
                                $style .= $this->curInputReadonly ? ' color:#555555;background-color:#DCDAD5' : '';
				$ret .= '<input style="'.$style.'" type="text" id="'.$inputID.'_text" name="'.$this->parseFieldName($inputName).'[text]" value="'.
								$this->getInputValue($curRowC, $isMulticolumn, 'text').
						'" '.($this->curInputReadonly?'readonly="readonly" ':'').$class.'/>'.$js."\n";
				$this->out .= $ret;
				break;
			}
			case 'date': {
				...
				$this->out .= '
					...
					<input type="hidden" id="'.$inputID.'" name="'.$this->parseFieldName($inputName).'" value="'.$default.'"/>
					'.$deleteButton.'
					<script type="text/javascript">/*<![CDATA[*/
                                        (function($) {
						$(document).ready(function() {
							...
							'.(!empty($default) ? '
							$("#'.$inputID.'_visual").val(
								jqml.datepicker.formatDate(dateFormat'.$this->curInput['key'].', new Date(\''.$default.'\'))
							);
							$("#'.$inputID.'").val(
			...
			case 'textarea': {
                if (    ('tinyMCE' == getDBConfigValue('general.editor',0,'tinyMCE'))
					...
					$this->out .= '
						...
						<script type="text/javascript">/*<![CDATA[*/
                                                (function($) {
							$(document).ready(function() {
								tinyMCE.init(jqml.extend(tinyMCEMagnaDefaultConfig, {
				...
				$this->out .= '
					<textarea type="text" id="'.$inputID.'" name="'.$this->parseFieldName($inputName).'" rows="10" '.$class.'>'.
	...
	private function renderField() {
		...
		$this->out .= '
			<tr class="'.(($this->oddEven = !$this->oddEven) ? 'odd' : 'even').'">
				<th colspan="2">'.fixHTMLUTF8Entities($this->curField['label']).''.($required ? ' <span>&bull;</span>' : '').'</th>
				<td class="input">'."\n";
		$this->renderInputs();
		$this->out .= '
				</td>
...
//			</tr>';
	...
	public function render() {
		...
		# echo print_m($this->failedItems);
		$hidden = '<input type="hidden" name="'.$this->parseFieldName('__'.get_class($this).'__').'" value="'.time().'"/>';
		foreach ($this->structure as $key => $block) {
			$hidden .= '<input type="hidden" name="'.$this->parseFieldName('__loadedBlocks').'[]" value="'.$key.'" />';
			$this->curBlock = $block;
			array_push($this->curProcItem, $key);
