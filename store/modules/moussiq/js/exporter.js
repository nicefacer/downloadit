...
{var collection=$('ul#export').find('li').not('.ui-sortable-helper');return collection.length>0?collection:false;}
    ,renderControls:function()
    {
        ...
        if($(this).hasClass('picture_link')){
            var fields={allPictures:fUrlsOfAllPictures};helper.createSetupInputFields(settingsDiv,fields,'checkbox');
            var fields={largePicture:fLargePicture};helper.createSetupInputFields(settingsDiv,fields,'checkbox');
        }
        if($(this).hasClass('description_short') || $(this).hasClass('description')){
            var fields={htmlTags:fHtmlTags}; helper.createSetupInputFields(settingsDiv,fields,'checkbox');
        }
...
{var field=$(this).attr('class').split(' ')[0],fields=helper.getInputVal($(this),['fName','fBefore','fAfter','fValue','fCodFee','withTax','withShipping','withReduction', 'allPictures', 'largePicture', 'htmlTags']);if(fields.fName.length==0)
{fields.fName=field;}
outputString+=helper.getjSonObject({field:field,fieldTitle:fields.fName,before:fields.fBefore,after:fields.fAfter,value:fields.fValue,withTax:fields.withTax,withShipping:fields.withShipping,withReduction:fields.withReduction,htmlTags:fields.htmlTags,largePicture:fields.largePicture,allPictures:fields.allPictures,fee:fields.fCodFee});});outputString=outputString.substr(0,parseInt(outputString.length-2));outputString+=']}';targetInput.val(outputString);}}
...
helper.setInputVal($(this),{fName:data[i].fieldTitle,fBefore:data[i].before,fAfter:data[i].after,fValue:data[i].value,fCodFee:data[i].fee,withTax:data[i].withTax,withShipping:data[i].withShipping,withReduction:data[i].withReduction, allPictures:data[i].allPictures, htmlTags:data[i].htmlTags, largePicture:data[i].largePicture});});}}
...
{var leftFieldset=$('fieldset.availFields'),rightFieldset=$('fieldset.targetFields'),fieldsList=rightFieldset.find('ul#export'),lHeight=leftFieldset.height(),rHeight=rightFieldset.height(),rTPadding=parseInt(rightFieldset.css('paddingTop')),rBPadding=parseInt(rightFieldset.css('paddingTop')),controlPH=rightFieldset.find('span.purgeFields').outerHeight(true),resHeight;resHeight=lHeight>rHeight?lHeight:rHeight;
    leftFieldset.css('height',resHeight+'px');
    rightFieldset.css('height',resHeight+'px');

    var headingHeight = rightFieldset.parent().parent().find('div.panel-heading').height();
    rightFieldset.parent().parent().css('height',resHeight+headingHeight+20+'px');
    rightFieldset.parent().css('height',resHeight+headingHeight+'px');

    fieldsList.css('height',(resHeight-(rTPadding+rBPadding+controlPH))+'px');
}
,init:function()
    {   exporter.fieldsetHeights();
        ...
        $("form#moussiq_service_form").submit(function(){exporter.updateAll();return true;});
    }
};
